...
- **Simple operational menu:** `autotranscribe menu` opens the `WatcherControl` menu with an always-visible compact `StatusSnapshot`, plus status, start/stop/restart, recent `TranscriptionJob`s, and the `LatestTranscript`.
- **Live status dashboard:** `npm run status` shows a terminal dashboard that refreshes every 500 ms (state, queue, current file; data from `runtime/status.json`). Press Ctrl+C to exit.
