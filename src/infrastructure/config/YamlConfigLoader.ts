...
function toBackendConfig(raw: any): BackendConfig {
  if (!raw) {
...
function toLoggingConfig(raw: any): LoggingConfig {
  if (!raw) {
    throw new Error("Missing 'logging' section in config.yaml");
  }

  const level = String(raw.level ?? "info") as LoggingConfig["level"];
  const validLevels: LoggingConfig["level"][] = ["debug", "info", "warn", "error"];
  if (!validLevels.includes(level)) {
    throw new Error(
      `Invalid logging.level '${level}' in config.yaml (expected one of ${validLevels.join(", ")})`
    );
  }
...
function toTitleConfig(raw: any): TitleConfig {
  ...
  if (provider === "ollama") {
    ...
    cfg.ollama = {
      ...
      timeoutMs: Number(o.timeout_ms ?? 120000)
    };
  }
