...
import type { JsonStatusSnapshot } from "../application/JsonStatusSnapshot.js";

const snapshot: JsonStatusSnapshot = {
...
test("SwiftBar menu truncates long errors", () => {
  const output = renderSwiftBarMenu(
    { ...snapshot, lastError: "x".repeat(200) },
...
test("SwiftBar menu reports missing or stale status without claiming the service is idle", () => {
  const output = renderSwiftBarMenu(
    { ...snapshot, statusFreshness: "missing", activity: null, latestTranscript: null },
    "/plugins/autotranscribe.5s.sh"
  );
