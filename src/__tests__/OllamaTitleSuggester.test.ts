...
import {
  ...
  probeOllamaTitleHealth
} from "../infrastructure/title/OllamaTitleSuggester.js";
import type { TitleConfig } from "../infrastructure/config/AppConfig.js";

const baseConfig: TitleConfig = {
  enabled: true,
  provider: "ollama",
  maxLength: 80,
  maxWords: 5,
...
function installFetchStub(stub: typeof fetch): () => void {
  ...
  try {
    ...
    await assert.rejects(
      ...
      (err: unknown) => {
        ...
        assert.match(
          err.message,
          /HTTP 404.*model 'llama3\.1:8b-instruct-q4_K_M' not found/
        );
        return true;
      }
    );
