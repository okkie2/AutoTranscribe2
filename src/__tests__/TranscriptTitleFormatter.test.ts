...
import {
  slugifyTitle,
  formatTranscriptWithTitle
} from "../application/TranscriptTitleFormatter.js";
...
test("formatTranscriptWithTitle prefixes content with H1 and builds filename", () => {
  ...
  const { title, finalContent, finalFileName } = formatTranscriptWithTitle(
    raw,
    "Kennismaking met Sabine",
    "2025-12-03_14-03-14"
  );

  assert.equal(title, "Kennismaking met Sabine");
  assert.ok(
    finalContent.startsWith("# Kennismaking met Sabine\n\nDit is de ruwe transcriptie."),
    "finalContent should start with H1 title followed by raw content"
  );
  assert.equal(
    finalFileName,
    "2025-12-03_14-03-14_kennismaking-met-sabine.md"
  );
});
