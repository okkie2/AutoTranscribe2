...
test("menu workflow handles status, start, restart, recent jobs, latest transcript, and stop", async () => {
  ...
  const latestTranscriptPath = path.join(
    rootDir,
    "transcripts",
  ...
  fs.writeFileSync(
    path.join(rootDir, "logs", "autotranscribe.log"),
    "[2026-03-27T09:20:00.000Z] [INFO] Finished transcription job {\"audioFile\":\"/tmp/a.m4a\",\"transcriptPath\":\"/tmp/a.md\",\"title\":\"Things to do tomorrow\"}\n",
    "utf8"
  );
  fs.writeFileSync(
    path.join(rootDir, "runtime", "status.json"),
    JSON.stringify({
  ...
  const child = spawn(
    process.execPath,
    [path.join(repoRoot, "dist/cli/index.js"), "menu"],
    {
      cwd: rootDir,
      env: {
        ...process.env,
  ...
  async function waitForOutput(pattern: RegExp, startIndex: number = 0): Promise<number> {
