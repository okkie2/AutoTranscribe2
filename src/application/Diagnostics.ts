...
function getDiagnosticBundleRoot(): string {
  return process.env.AUTOTRANSCRIBE_DIAGNOSTIC_BUNDLE_DIR ||
    path.join(os.homedir(), "Library", "Logs", "AutoTranscribe2");
}

export function exportDiagnosticBundle(config: AppConfig): DiagnosticBundle {
  const bundlePath = path.join(
    getDiagnosticBundleRoot(),
    `diagnostic-bundle-${sanitizeTimestamp(new Date())}`
  );
