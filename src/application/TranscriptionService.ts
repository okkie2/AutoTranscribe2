...
import type { TitleConfig } from "../infrastructure/config/AppConfig.js";
import { formatTranscriptWithTitle } from "./TranscriptTitleFormatter.js";
import type { TitleProviderState } from "../infrastructure/status/RuntimeStatus.js";

interface ParsedBackendTranscript {
  rawText: string;
  formattedBody: string;
  detectedLanguage: string | null;
}

interface TranscriptVariant {
  languageHint: string | null;
  parsed: ParsedBackendTranscript;
  score: number;
}

interface TitleProviderStatusUpdate {
  titleProviderState: TitleProviderState;
  titleProviderDetail: string | null;
}

const FALLBACK_LANGUAGES = ["nl", "en"] as const;
...
function parseBackendTranscript(backendContent: string): ParsedBackendTranscript {
...
function computeLongestRepeatedRun(tokens: string[]): number {
  if (tokens.length === 0) {
    return 0;
  }
...
function computeRepeatedTrigramRatio(tokens: string[]): number {
  if (tokens.length < 6) {
    return 0;
  }
...
function computeStopwordRatio(tokens: string[], language: "nl" | "en"): number {
  if (tokens.length === 0) {
    return 0;
  }
...
function scoreTranscriptText(text: string, preferredLanguage: "nl" | "en"): number {
  ...
  return (
    Math.min(tokens.length, 400) * 0.03 +
    stopwordRatio * 60 +
    uniqueRatio * 18 -
...
function bestLanguageAgnosticScore(text: string): number {
  return Math.max(scoreTranscriptText(text, "nl"), scoreTranscriptText(text, "en"));
}

function shouldRetryWithLanguageFallback(text: string): boolean {
  const tokens = tokenizeWords(text);
  if (tokens.length < 12) {
    return false;
  }
...
export class TranscriptionService {
  constructor(
    ...
    private readonly updateTitleProviderStatus?: (update: TitleProviderStatusUpdate) => void
  ) {}

  private async transcribeVariant(audioFile: AudioFile, languageHint: string | null): Promise<TranscriptVariant> {
  ...
  private async selectTranscriptVariant(audioFile: AudioFile, languageHint: string | null): Promise<TranscriptVariant> {
    ...
    const candidates: TranscriptVariant[] = [primary];
    ...
    if (this.titleConfig.enabled && this.titleConfig.provider !== "none") {
      try {
        ...
        this.updateTitleProviderStatus?.({
          titleProviderState: "ready",
          titleProviderDetail:
            this.titleConfig.provider === "ollama"
