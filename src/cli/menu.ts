...
import type { EffectiveState } from "../application/StatusSnapshot.js";
...
function printCompactStatusSnapshot(config: AppConfig): void {
  for (const line of getCompactStatusSnapshot(config)) {
    console.log(line);
  }
}

function printStatusSnapshot(config: AppConfig): void {
...
function colorizeState(state: EffectiveState | "none"): (text: string) => string {
  switch (state) {
    case "idle":
      return chalk.green;
    case "processing":
...
function renderLiveStatus(config: AppConfig): void {
  const snapshot = getStatusSnapshot(config);
...
function printRecentJobs(config: AppConfig): void {
...
): Promise<{ running: boolean; skipPause?: boolean }> {
  switch (selection) {
    case "1":
