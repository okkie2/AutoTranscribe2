...
function scoreCandidate(tokens, freq) {
    let score = 0;
    let contentCount = 0;
    ...
    for (const t of tokens) {
        ...
        if (!isStopword(t) && !isFiller(t)) {
            ...
            score += f;
            if (t.length >= 5)
                strongContentCount += 1;
            if (t.length >= 5 && f > 0 && f <= 3)
                rareContentCount += 1;
        }
    ...
    if (contentCount >= 2)
        score += 3;
    if (strongContentCount >= 1)
        score += 2;
    if (rareContentCount >= 1)
        score += 4;
    if (hasProperNoun)
        score += 2;
    return score;
}
function trimStopwordsAtEdges(tokens) {
    let start = 0;
    let end = tokens.length;
    while (end - start > 1 && (isStopword(tokens[start]) || isFiller(tokens[start])))
        start += 1;
    while (end - start > 1 && (isStopword(tokens[end - 1]) || isFiller(tokens[end - 1])))
        end -= 1;
    return tokens.slice(start, end);
}
export class HeuristicTitleSuggester {
