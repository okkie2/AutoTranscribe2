...
import { TranscriptionJobLedger, rehydrateRecoverableJobs } from "../infrastructure/jobs/TranscriptionJobLedger.js";
...
test("queued transcription jobs are recovered from the durable ledger after restart", async () => {
    ...
    const worker = new JobWorker(recoveredQueue, service, logger, createStatusUpdater(statusPath), { idlePollIntervalMs: 10, heartbeatIntervalMs: 25, jobLedger: ledger });
    const controller = new AbortController();
    const workerPromise = worker.start(controller.signal);
    const expectedTranscriptPath = path.join(transcriptsDir, "2026-03-27_10-30-00_recovered-title.md");
...
test("in-progress jobs are rehydrated as pending work after restart", () => {
    ...
    const previousUpdatedAt = new Date(Date.now() - 60000);
    const ledger = new TranscriptionJobLedger(ledgerPath);
    ledger.record({
        id: "job-in-progress",
