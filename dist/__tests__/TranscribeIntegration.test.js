...
test("CLI transcribe creates a titled markdown transcript for the fixture", (t) => {
    ...
    const fixturePath = path.resolve(projectRoot, "test", "fixtures", `${baseName}.m4a`);
    ...
    const transcriptsForBase = after.filter((name) => name.startsWith(baseName) && name.toLowerCase().endsWith(".md"));
    assert.equal(transcriptsForBase.length, 1, `Expected exactly one transcript for ${baseName}, found ${transcriptsForBase.length}`);
    const transcriptPath = path.join(transcriptsDir, transcriptsForBase[0]);
    const content = fs.readFileSync(transcriptPath, "utf8");
    const firstLine = content.split(/\r?\n/, 1)[0];
    assert.ok(firstLine.startsWith("# "), `Expected first line to start with '# ', got: ${firstLine}`);
});
