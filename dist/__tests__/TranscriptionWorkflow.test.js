...
Remember to water the plants and take out the recycling.`,
    ...
    }, queue, logger, null, createStatusUpdater(runtimeStatusPath));
    poller.scanOnce();
    assert.equal(queue.getLength(), 0);
    poller.scanOnce();
    assert.equal(queue.getLength(), 1);
    const worker = new JobWorker(queue, service, logger, createStatusUpdater(runtimeStatusPath), { heartbeatIntervalMs: 25, idlePollIntervalMs: 10 });
    const controller = new AbortController();
    const workerPromise = worker.start(controller.signal);
    const transcriptPath = path.join(transcriptsDir, "2026-03-27_09-15-00_things-to-do-tomorrow.md");
